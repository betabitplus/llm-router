# %%
"""Gemini WebAPI experiment tool-loop helpers.
...
When to use:
    Import from Gemini WebAPI experiment scripts that need to parse exact
...
def parse_tool_call(
    ...
    *,
    ...
    """Parse one observed textual or JSON-encoded Gemini WebAPI tool call."""
    ...
    if cleaned_text.startswith("{"):
        ...
        if isinstance(payload, dict):
            ...
            if isinstance(tool_name, str) and isinstance(raw_arguments, dict):
                return {
                    "tool_name": tool_name,
                    "arguments": dict(raw_arguments),
                }

    match = _CALL_RE.fullmatch(cleaned_text)
    if match is None:
        ...
        for item in raw_args.split(","):
            ...
            if positional_index >= len(positional_names):
                msg = (
                    ...
                    f"experiment parser supports: {text!r}"
                )
                raise RuntimeError(msg)
...
def demo_add(*, a: int, b: int) -> int:
    """Return a+b as the shared Gemini WebAPI demo tool result."""
    return a + b
