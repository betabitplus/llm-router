# %%
"""Gemini WebAPI async nonstandard structured tool-loop experiment script.
...
Examples:
    Run manually:
        uv run python -m experiments.llm_router.gemini_webapi.tool_loop_structured_async
        uv run python scripts/reproduce_running_loop.py \
            experiments.llm_router.gemini_webapi.tool_loop_structured_async
"""
...
async def run_pipeline() -> dict[str, Any]:
    ...
    # Ask the model for the next function call in text, execute that call locally,
    # then feed the concrete tool result back into the same conversation flow.
    async with managed_client(init_timeout_seconds=_INIT_TIMEOUT_SECONDS) as client:
        ...
        trace = [first_call]
        # Some live responses finish after one tool; otherwise treat the second
        # response as another call and continue until we can validate final JSON.
