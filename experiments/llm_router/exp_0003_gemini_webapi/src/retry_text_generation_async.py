# %%
"""Gemini WebAPI async retry text-generation experiment script.
...
Examples:
    From this capsule root:
        uv run --locked --managed-python python src/retry_text_generation_async.py
"""
...
async def run_pipeline() -> dict[str, Any]:
    ...
    # Keep the authenticated browser-backed provider call real, but wrap only the
    # generation request so session startup is not mistaken for retry evidence.
    retry_events = []
    attempt_count = 0
    ...
    # Keep the response and retry telemetry together so the report shows both the
    # capability result and whether this particular live run actually retried.
    return {
        "model": _MODEL,
