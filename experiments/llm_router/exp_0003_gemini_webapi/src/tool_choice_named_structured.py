# %%
"""Gemini WebAPI nonstandard forced-tool-choice structured experiment script.
...
Examples:
    Run manually:
        uv run python -m \
            experiments.llm_router.gemini_webapi.tool_choice_named_structured
...
async def run_pipeline() -> dict[str, Any]:
    ...
    # Ask for one specific local function first; Gemini WebAPI has no native tool
    # protocol here, so the provider must express the call in model text.
    async with managed_client(init_timeout_seconds=_INIT_TIMEOUT_SECONDS) as client:
        ...
        if first_text.startswith("{"):
            first_payload = parse_model_output_json(first_text)
            if "final_result" in first_payload and "explanation" in first_payload:
        ...
        # Execute the chosen function locally, feed its concrete result back to the
        # model, and require a typed final JSON answer rather than free-form prose.
