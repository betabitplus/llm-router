# %%
"""QwenChat async retry text-generation experiment script.
...
Examples:
    Run manually:
        uv run python -m experiments.llm_router.qwenchat.retry_text_generation_async
        uv run python scripts/reproduce_running_loop.py \
            experiments.llm_router.qwenchat.retry_text_generation_async
"""
...
from _structured_output import normalize_reply
from _uploads import build_user_content_async
from py_lib_testkit import console, run_async


class _RetryableProxyError(RuntimeError):
    """Mirror the adapter's retryable proxy-status branch for experiment demos."""
