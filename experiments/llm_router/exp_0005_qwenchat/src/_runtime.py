# %%
"""QwenChat experiment runtime helpers.
...
When to use:
    Import from QwenChat experiment helpers or scripts that need the configured
...
def qwenchat_base_url() -> str:
    """Return the QwenChat base URL for this experiment suite."""
...
def build_sync_client() -> httpx.Client:
    """Build one sync HTTP client for QwenChat experiment calls."""
    return httpx.Client(trust_env=False, timeout=_DEFAULT_TIMEOUT_SECONDS)


def build_async_client() -> httpx.AsyncClient:
    """Build one async HTTP client for QwenChat experiment calls."""
