# %%
"""Internal AI Studio experiment SDK helpers.

Why:
    Keeps repeated OpenAI-compatible client creation, response parsing, and
    image encoding in one place so the AI Studio experiment scripts stay focused
    on one provider seam each.

When to use:
    Import from AI Studio experiment scripts that exercise the non-video
...
def openai_base_url() -> str:
    """Return the OpenAI-compatible AI Studio base URL for experiment runs."""
    override = os.getenv("WORKBENCH_AISTUDIO_OPENAI_BASE_URL")
    return override or _DEFAULT_OPENAI_BASE_URL


def api_key_env_name() -> str:
    """Return the preferred env var for live AI Studio experiment runs."""
