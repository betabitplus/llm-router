# %%
"""AI Studio experiment JSON-schema helpers.

Why:
    AI Studio can mishandle `$ref` and `$defs` on the OpenAI-compatible path,
    so the experiment needs one explicit place to inline references before a
    live structured-output request.

When to use:
    Import from AI Studio experiment scripts that need a resolved
