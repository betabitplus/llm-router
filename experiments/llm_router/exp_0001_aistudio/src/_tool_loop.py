# %%
"""AI Studio experiment tool-loop helpers.

Why:
    AI Studio needs one nonstandard tool-loop path when tools and structured
    output are requested together: run tools first, then ask for the final JSON
    in a second call.

When to use:
    Import from AI Studio experiment scripts that need forced tool choice or
    the tools-plus-structured two-step flow.
"""
...
class FunctionToolCall(TypedDict):
    """One parsed function call emitted by the AI Studio response."""
...
class ToolTraceEntry(TypedDict):
    """One locally executed tool step recorded for experiment output."""
...
class ToolMessagePayload(TypedDict, total=False):
    """One AI Studio tool-role message payload appended after execution."""
...
class ToolLoopResult(TypedDict, total=False):
    """JSON-ready evidence returned by the AI Studio tool loop."""
...
def demo_add(*, a: int, b: int) -> dict[str, int]:
    """Return a+b as the shared AI Studio demo tool payload."""
    return {"result": a + b}
...
def build_demo_math_tools() -> list[dict[str, Any]]:
    """Build the shared add/multiply tool declarations for AI Studio demos."""
    return [
        build_function_tool(
            name="add",
...
def demo_math_registry() -> dict[str, Callable[..., Any]]:
    """Return the shared local registry for the AI Studio demo math tools."""
    return {
        "add": demo_add,
...
def assistant_message_dict(response: ChatCompletion) -> AssistantMessagePayload:
    ...
    if raw_calls:
        # Keep provider-specific extension fields such as Gemini thought signatures.
        # Reconstructing only the OpenAI-standard fields makes the follow-up request
        # invalid once AI Studio requires the signature on returned function calls.
        payload["tool_calls"] = [
            dict(tool_call.model_dump(exclude_none=True)) for tool_call in raw_calls
        ]
...
def tool_message_dict(
    *,
    ...
    """Build one AI Studio tool-role message for a local tool result."""
...
def run_sync_tool_loop(  # noqa: PLR0913
    *,
    ...
    """Run a sync AI Studio tool loop and return JSON-ready evidence."""
    ...
    msg = (
        "The live AI Studio tool loop did not finish within the configured round limit."
    )
...
async def run_async_tool_loop(  # noqa: PLR0913
    *,
    ...
    """Run an async AI Studio tool loop and return JSON-ready evidence."""
    messages: list[dict[str, Any]] = [{"role": "user", "content": prompt}]
    trace: list[ToolTraceEntry] = []
    current_tool_choice = tool_choice
    ...
    msg = (
        "The live async AI Studio tool loop did not finish within the configured "
        "round limit."
    )
    raise RuntimeError(msg)
