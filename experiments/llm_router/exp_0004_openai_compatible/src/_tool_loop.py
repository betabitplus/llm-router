# %%
"""OpenAI-compatible experiment tool-loop helpers.

Why:
    Keeps the multi-round tool protocol in one place so the experiment scripts
    can inspect real tool behavior without depending on `src/` internals.

When to use:
    Import from OpenAI-compatible experiment scripts that need tool definition
...
from _sdk_helpers import parse_message_json, response_text
...
def demo_add(*, a: int, b: int) -> dict[str, int]:
    """Return a+b as the shared experiment demo tool payload."""
    return {"result": a + b}
...
def run_sync_tool_loop(  # noqa: PLR0913
    *,
    ...
    for _ in range(max_rounds):
        ...
        if isinstance(current_tool_choice, dict) or current_tool_choice in (
            "required",
            "any",
        ):
