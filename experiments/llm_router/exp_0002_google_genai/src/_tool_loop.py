# %%
"""Google GenAI experiment tool-loop helpers.

Why:
    Keeps the native function-call loop in one place so multiple experiment
    scripts can inspect tool behavior without depending on `src/` internals.

When to use:
    Import from Google GenAI experiment scripts that need callable declarations,
...
from _sdk_helpers import parsed_response_dict
