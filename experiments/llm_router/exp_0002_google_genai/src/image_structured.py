# %%
"""Google GenAI image structured-output experiment script.
...
Examples:
    Run manually:
        uv run python -m experiments.llm_router.google_genai.image_structured
        uv run python scripts/reproduce_running_loop.py \
            experiments.llm_router.google_genai.image_structured
"""
...
from experiment_support import build_test_image
...
# Keep one stable multimodal smoke model and the shared image fixture fixed so
# the direct-image SDK seam stays easy to inspect.
...
def run_pipeline() -> dict[str, Any]:
    ...
    # Build the native SDK client exactly as this experiment family uses it.
    client = build_client()
