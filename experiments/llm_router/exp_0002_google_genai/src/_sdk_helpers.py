# %%
"""Internal Google GenAI experiment SDK helpers.

Why:
    Keeps repeated client creation, parsed-output handling, and usage snapshots
    in one place so the Google GenAI experiment scripts stay focused on one SDK
    seam each.

When to use:
    Import from Google GenAI experiment scripts that make live SDK requests and
