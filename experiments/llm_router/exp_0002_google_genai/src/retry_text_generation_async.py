# %%
"""Google GenAI async retry text-generation experiment script.
...
Examples:
    Run manually:
        uv run python -m experiments.llm_router.google_genai.retry_text_generation_async
        uv run python scripts/reproduce_running_loop.py \
            experiments.llm_router.google_genai.retry_text_generation_async
"""
...
async def run_pipeline() -> dict[str, Any]:
    ...
    # Keep the provider call real, but wrap only that call in the retry policy so
    # the report can distinguish configured retry behavior from observed attempts.
    client = build_client()
    retry_events = []
    attempt_count = 0
    ...
    # Preserve both the useful provider reply and the retry telemetry; a successful
    # first attempt is evidence that the path works, not evidence that retries fired.
    return {
        "model": _MODEL,
