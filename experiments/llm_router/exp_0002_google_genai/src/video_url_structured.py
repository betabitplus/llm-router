# %%
"""Google GenAI remote-video structured-output experiment script.
...
Examples:
    Run manually:
        uv run python -m experiments.llm_router.google_genai.video_url_structured
        uv run python scripts/reproduce_running_loop.py \
            experiments.llm_router.google_genai.video_url_structured
"""
...
from _media_parts import build_video_url_part
...
# Keep one stable multimodal smoke model and one fixed public video URL so the
# remote file-data request shape is the main thing under inspection.
