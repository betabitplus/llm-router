# %%
"""LLM Router e2e: QwenChat local video + structured output.

Why:
    Verifies that QwenChat supports local video input with structured output.
...
Examples:
    Run manually:
        uv run python -m \
            tests.llm_router.e2e.provider_sdk_wrapping.test_qwenchat_video_file_structured_pipeline

    Run as test:
        pytest \
            tests/llm_router/e2e/provider_sdk_wrapping/test_qwenchat_video_file_structured_pipeline.py
"""
...
# One fixed clip keeps this scenario about QwenChat's local-video path rather
# than changing clip semantics.
...
def run_pipeline(*, video: VideoSchema) -> LLMRouterResponse:
    """Run the QwenChat local-video pipeline."""
    # This is the public workflow we care about: prompt plus one local video.
    router = build_router()
    return router.query(
...
def main() -> None:
    ...
    console.demo_step(
        ...
        "We upload a local video file and ask QwenChat to summarize it "
