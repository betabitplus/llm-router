...
| `VC_CONFIG_RETRY_WAIT_BOUNDS`               | {need}`[[id]] <TREQ_CONFIG_RETRY_WAIT_BOUNDS>`            | Component  | Local    |              2 | `min-wait-non-positive` · `max-wait-below-min`                  | Non-positive minimum wait and maximum-wait-below-minimum configurations are both rejected.                          |
...
| `VC_CONFIG_MODEL_PROVIDER_REFERENCES`       | {need}`[[id]] <TREQ_CONFIG_MODEL_PROVIDER_REFERENCES>`    | Component  | Local    |              2 | `empty-model-mapping` · `undeclared-provider-reference`         | Empty model mappings and mappings to providers absent from the catalog are both rejected.                           |
