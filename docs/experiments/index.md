...
Each Evidence cell is executed during an explicit live capture and tagged `hide-input`. Stored outputs use standard notebook and Sphinx primitives: Sphinx Design cards for **Input** and **Observed output**, stock dropdowns for the actual executed provider code and the complete raw result, native Sphinx/Pygments JSON highlighting inside the raw-result dropdown, and native IPython image/PDF/video representations for retained media. DocOps supplies no experiment-specific presentation framework or JavaScript for experiment rendering.
