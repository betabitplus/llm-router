...
| Freshness states (`CURRENT / STALE / UNKNOWN`)                                                          | A semantics + B policy     | project Test Plan defines what counts as current                                                                     | same retained execution window plus byte-for-byte run-start verification-input snapshot; changed inputs make evidence stale                                       | `ternforge-infra-ci` retention + `ternforge-tooling-docops` projection   |
