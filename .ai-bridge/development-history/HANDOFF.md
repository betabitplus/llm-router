...
Overall — круг: вердикт в центре, кольца иерархии, кольцо контрактов и по кольцу на слой; каждый луч — один контракт через все слои. Каждый слой называет причины своих провалов, у вида есть адрес `#вид:ID`, страницы контрактов, целей и способностей ведут на своё место на карте, карточки показывают изменения с прошлого удержанного прогона (история 041).

С истории 044 это единственная карта. Сначала она монитор здоровья; меры рядом со здоровьем слоя нужны, чтобы при беде тут же увидеть, где, насколько и куда нырять за подробностями, не уходя на другую страницу.

### Меры рядом со здоровьем (бывшая Depth Map)

Меры — виды тех слоёв, на чей вопрос они отвечают:

- Depth у Overall: до какого уровня тестов доходят собственные тесты контракта и с чем они говорят на границе системы, лучами через кольца уровней;
- Test level у Coverage: самый глубокий уровень собственных тестов;
- Boundary у Coverage: ничего, substitute, запись или живой сервис;
- Model validation у Evidence quality: насколько проверены модели за substitutes и записями (L0–L4);
- Mutants caught у Fault model: какую долю мутантов своего кода ловят его тесты.

Меры измеряют, а не судят: последовательные шкалы без красного и зелёного, «не измерено» — штриховкой (истории 042, 044).

### Одна карта из трёх частей

Страница запускает один общий runtime `mapPage` из `assurance_map_pages.py`: виды, лента слоёв, All layers, Filters, таблица, карточка, Find, адрес, клавиши, Changes и раскладка у всех видов общие. Части только описывают свои слои: здоровье (`healthMap`) судит, меры (`depthMap`) измеряют, сведение (`pairsMap`) ставит меры видами на карточки слоёв здоровья и строит одну таблицу контрактов и одну CSV. Различия частей допустимы только смысловые. Гейт проверяет, что части не собирают общие куски сами (истории 042–044).

Старые адреса Depth Map и прототипа (`verification-depth-map.html`, `verification-map.html`) удалены; ссылки на меры ведут на виды Health Map: `#overall/depth:ID`, `#faults/detect:ID` (история 044).
...
Образец того, как глубоко прорабатывать страницу, — эталоны в [`../exemplars/`](../exemplars/README.md); первый из них — [Verification Health Map](../exemplars/001-verification-health-map.md) с чек-листом для следующих страниц.
...
- отдельные страницы здоровья и мер или прототип рядом с принятой картой: за одним ответом пришлось бы ходить дважды (044).
