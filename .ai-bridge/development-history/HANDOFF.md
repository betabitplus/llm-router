...
Он не заменяет repository state и не заменяет историю 001–038. Его задача — передать то, что новому человеку опаснее всего потерять: **ментальную модель, принятые границы, инварианты, UX-требования и выученные уроки**.
...
- Depth Map — compact Test Strength projection;
...
- как далеко reach;
- насколько reality/fidelity;
- каков Test Strength;
- где surrogate evidence зависит от producer credibility.
...
Если текущая задача — локальный prototype, shared repos не трогаются.
...
N/A визуально приглушён и не конкурирует с PASS/FAIL.
...
Не обязательно читать 001–038 подряд.
